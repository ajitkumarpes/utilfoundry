...
type Result = { url?: string; text?: string; name?: string; size?: number; mime?: string; width?: string; height?: string };

const groups = ["All", "Optimize", "Transform", "Convert", "Privacy"] as const;
...
export function ImageWorkbench() {
  ...
  const [options, setOptions] = useState<Record<string, string>>({ quality: "78", format: "webp", width: "1600", height: "", left: "0", top: "0", angle: "90", direction: "horizontal" });
  ...
  async function run() {
    ...
    try {
      ...
      if (contentType.includes("application/json")) {
        const body = await response.json() as { data: string; bytes: number };
        setResult({ text: body.data, size: body.bytes, mime: contentType });
      } else {
  ...
  return (
    <div className="app-shell">
      ...
      <main id="top" className="main-content">
        ...
        <section className="hero"><div><span className="eyebrow"><Sparkles size={14} /> BUILT FOR CLEANER MEDIA</span><h1>Image work,<br /><em>without the busywork.</em></h1><p>Convert, compress, resize and protect images with predictable output, clear controls and no account required.</p></div><div className="hero-stat"><strong>{TOOLS.length}</strong><span>focused tools</span><i><Check size={14} /> no AI in the core pipeline</i></div></section>
        ...
        <section className="workspace" aria-label="Image workbench">
          ...
          {result && <div className="result-panel"><div className="result-heading"><div><span className="section-kicker">Output</span><h3>Ready to use</h3></div><span>{result.size ? `${(result.size / 1024).toFixed(1)} KB` : ""}</span></div>{result.text ? <div className="text-result"><textarea readOnly value={result.text} /><div><button onClick={() => copy(result.text ?? "")}><Copy size={15} /> Copy</button></div></div> : <div className="result-content">{isImageResult && result.url ? <img src={result.url} alt="Processed image preview" /> : <div className="pdf-result"><FileImage size={30} /><strong>PDF generated</strong><span>{result.name}</span></div>}<a className="download-button" href={result.url} download={result.name}><Download size={16} /> Download output</a></div>}</div>}
        </section>
