...
This Compose project runs the three UtilFoundry surfaces and the complete PDF runtime behind Caddy:

- `utilfoundry.com` → the landing page
- `pdf.utilfoundry.com` → the PDF frontend, with `/api/*` and `/actuator/*` forwarded to Spring
- `dev.utilfoundry.com` → the developer-tools application
...
5. Verify `https://utilfoundry.com`, `https://pdf.utilfoundry.com`, and `https://dev.utilfoundry.com` before opening traffic.
