...
export default function ExtractAttachmentsPage() {
  ...
  const [resultFilename, setResultFilename] = useState("attachments.zip");
  ...
  const submit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/v1/pdf/extract-attachments`, {
        method: "POST",
        body: formData
      });
      ...
      const filename = parseFilename(
        ...
        isZip ? "attachments.zip" : "attachment"
      );
  ...
  return (
    <main className="tool-page">
      ...
      <section className="tool-hero">
        ...
        <h1>Extract Attachments</h1>
        <p>Pull every file embedded inside a PDF back out — document-level attachments and page-pinned files alike.</p>
      </section>
      ...
      <section className="workspace">
        ...
        {file && !done && (
          <div className="file-panel" style={{ marginTop: 14 }}>
            <button type="button" className="primary-btn wide" disabled={processing} onClick={submit}>
              {processing ? "Extracting…" : "Extract Attachments"}
            </button>
          </div>
        )}
        ...
        {done && (
          <div className="result-panel">
            ...
            <h2>Your attachments are ready</h2>
            <p>
              {resultIsZip
                ? "Every embedded file was packed into a ZIP. Your download should start automatically."
                : "The embedded file was extracted. Your download should start automatically."}
            </p>
